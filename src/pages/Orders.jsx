...
const Orders = () => {
  ...
  const cancelReasons = [
    ...
    { id: "2", label: "Better price available elsewhere", icon: BadgeDollarSign },
  ...
  // Jab banao, POST/PATCH /api/orders/cancel { orderNumber, itemId, reason }
  // jaisa kuch expect kar raha hoon — endpoint milte hi yahan URL match kar lena.
  const handleCancelOrder = async () => {
    ...
    try {
      setCancelling(true);
      const data = await apiHelper.patch("/web/orders/cancel", {
        orderNumber: selectedItem.orderId,
            ...
            <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh] md:max-h-[85vh] lg:max-h-[80vh]">
              <div className="relative bg-gradient-to-r from-green-600 to-green-700 px-4 sm:px-6 py-4 text-white flex-shrink-0">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <h3 className="text-lg sm:text-xl font-bold text-black">Cancel Item</h3>
                  ...
                  <div className="mb-4 sm:mb-5">
                    <div className="bg-green-50 border border-green-200 rounded-2xl p-3 flex items-start gap-3">
                      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-green-100 flex items-center justify-center flex-shrink-0">
                        {(() => {
                          const selected = cancelReasons.find((r) => r.label === cancelReason);
                          if (!selected) return null;
                          const Icon = selected.icon;
                          return <Icon className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-green-600" />;
                        })()}
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-green-600 mt-0.5 sm:mt-1">{cancelReason}</p>
                      </div>
                    </div>
                  </div>
                )}
      ...
      <div className="bg-white sticky top-0 z-10">
        <div className="w-full xl:max-w-[1600px] 2xl:max-w-[1720px] mx-auto px-4 sm:px-6 lg:px-20 xl:px-24 2xl:px-46 pt-12 md:pt-16 lg:pt-20">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight mb-2">
            My <span className="text-transparent bg-clip-text bg-green-600">Orders</span>
          </h1>
          <p className="text-sm text-green-600 mt-1">Track each product in your order separately</p>
        </div>
      </div>
              ...
              <div key={order.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        <span className="flex items-center gap-1">
                          <IndianRupee className="h-3 w-3" />₹{order.total.toLocaleString()}
                        </span>
                ...
                <div className="divide-y divide-gray-100">
                  {order.items.map((item) => (
                    <div key={item.id} className="group bg-white hover:bg-gray-50/60 transition-all duration-300">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                          <div className="lg:col-span-5 flex items-center gap-4">
                              ...
                              <div
                                className={`absolute -top-1 -right-1 w-5 h-5 rounded-full border-2 border-white shadow ${
                                  item.tracking.status === "Delivered"
                                    ...
                                    ? "bg-blue-500"
                                    : item.tracking.status === "Out for Delivery"
                                    ? "bg-orange-500"
                            ...
                            <div className="min-w-0">
                              <h3 className="text-[15px] sm:text-lg font-semibold text-black">{item.name}</h3>
                              ...
                              <div className="mt-3">
                                {item.tracking.status === "Delivered" ? (
                                  ...
                                  <div className="flex items-center gap-2 text-red-600">
                                    ...
                                    <p className="text-sm font-medium">Cancelled • Refund Initiated</p>
                                  </div>
                                ) : (
                                  <div className="flex items-center gap-2 text-blue-600">
                                    <Truck className="h-4 w-4" />
                                    <p className="text-sm font-medium">
                                      Expected delivery: {item.tracking.estimated}
                                    </p>
                                  </div>
                                )}
                              </div>
                          ...
                          <div className="lg:col-span-3 flex flex-row lg:flex-col items-center lg:items-end gap-3">
                            <div
                              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full whitespace-nowrap text-xs font-semibold ${getStatusBadge(
                            ...
                            {item.tracking.status !== "Delivered" && item.tracking.status !== "Cancelled" && (
                              <button
                                onClick={() => openCancelModal(order.id, item)}
