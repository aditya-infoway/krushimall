...
// ---- Option lists ----
// NOTE: Exact tyre condition labels were not specified — assuming
// New / Good / Average / Worn Out. Update if your actual list differs.
...
const hydraulicConditionOptions = [
...
const attachmentFields = [
  ["attachments.rotavator", "Rotavator"],
  ["attachments.cultivator", "Cultivator"],
  ["attachments.trailer", "Trailer"],
  ["attachments.trolley", "Trolley"],
  ["attachments.mbPlough", "MB Plough"],
  ["attachments.seedDrill", "Seed Drill"],
  ["attachments.sprayer", "Sprayer"],
  ["attachments.dozer", "Dozer"],
  ["attachments.loader", "Loader"],
];
...
export default function HydraulicTyres({
  ...
  useEffect(() => {
    ...
    reset({
      frontTyreBrand: productData.frontTyreBrand ?? "",
      frontTyreRemainingPercent: productData.frontTyreRemainingPercent ?? "",
      frontTyreCondition: productData.frontTyreCondition ?? "",

      rearTyreBrand: productData.rearTyreBrand ?? "",
      rearTyreRemainingPercent: productData.rearTyreRemainingPercent ?? "",
      rearTyreCondition: productData.rearTyreCondition ?? "",

      hydraulicLiftWorking: productData.hydraulicLiftWorking ?? "",
      hydraulicCondition: productData.hydraulicCondition ?? "",

      ptoStatus: productData.ptoStatus ?? "",

      attachments: {
        rotavator: productData.rotavator ?? false,
        cultivator: productData.cultivator ?? false,
        trailer: productData.trailer ?? false,
        trolley: productData.trolley ?? false,
        mbPlough: productData.mbPlough ?? false,
        seedDrill: productData.seedDrill ?? false,
        sprayer: productData.sprayer ?? false,
        dozer: productData.dozer ?? false,
        loader: productData.loader ?? false,
      },
    });
  }, [productData, reset]);
  ...
  ? productData.id
  : localStorage.getItem("vendorProductId");   // was: localStorage.getItem(...) || productData?.id;
...
if (!productId) {
  ...
  return;
      ...
      const payload = {
        frontTyreBrand: data.frontTyreBrand,
        frontTyreRemainingPercent: data.frontTyreRemainingPercent
          ? Number(data.frontTyreRemainingPercent)
          : null,
        frontTyreCondition: data.frontTyreCondition,

        rearTyreBrand: data.rearTyreBrand,
        rearTyreRemainingPercent: data.rearTyreRemainingPercent
          ? Number(data.rearTyreRemainingPercent)
          : null,
        rearTyreCondition: data.rearTyreCondition,

        hydraulicLiftWorking: data.hydraulicLiftWorking,
        hydraulicCondition: data.hydraulicCondition,
    ...
    <div className="min-h-screen bg-gray-50 py-8 md:py-12 lg:py-16">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Provide tyre condition, hydraulics, PTO and attachment details
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
            <div className="p-6 md:p-8 lg:p-10 space-y-10">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                  <Input
                    {...register("frontTyreBrand")}
                    type="text"
                    label="Brand"
                    placeholder="Enter tyre brand"
                    error={errors?.frontTyreBrand?.message}
                  />
                  <Input
                    {...register("frontTyreRemainingPercent")}
                    type="number"
                    label="Remaining %"
                    placeholder="Enter remaining %"
                    error={errors?.frontTyreRemainingPercent?.message}
                  />
                  <SelectField
                    control={control}
                    errors={errors}
                    name="frontTyreCondition"
                    data={tyreConditionOptions}
                    label="Condition"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                  <Input
                    {...register("rearTyreBrand")}
                    type="text"
                    label="Brand"
                    placeholder="Enter tyre brand"
                    error={errors?.rearTyreBrand?.message}
                  />
                  <Input
                    {...register("rearTyreRemainingPercent")}
                    type="number"
                    label="Remaining %"
                    placeholder="Enter remaining %"
                    error={errors?.rearTyreRemainingPercent?.message}
                  />
                  <SelectField
                    control={control}
                    errors={errors}
                    name="rearTyreCondition"
                    data={tyreConditionOptions}
                    label="Condition"
                    placeholder="Select condition"
                  />
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <SelectField
                    ...
                    data={hydraulicConditionOptions}
