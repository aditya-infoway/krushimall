...
const hydraulicTypes = [
  { label: "ADDC", value: "addc", description: "Automatic Depth & Draft Control", icon: GitBranch },
  { label: "Position Control", value: "positionControl", description: "Maintain implement position", icon: Tractor },
  { label: "Draft Control", value: "draftControl", description: "Automatic draft sensing control", icon: Settings },
];
const controlTypeOptions = [
  { label: "Automatic Control", value: "automatic", description: "Automatic response and easy operation", icon: Settings },
  { label: "Manual Control", value: "manual", description: "Manual operation and full control", icon: Joystick },
];
const remoteValveOptions = [
  { label: "Single Acting", value: "single_acting", description: "For single direction operation" },
  { label: "Double Acting", value: "double_acting", description: "For both direction operation" },
];
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 md:py-12 lg:py-16">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Hydraulic & Tyre Details</h1>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
            <div className="p-6 md:p-8 lg:p-10 space-y-10">
              {/* Lifting Capacity */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-6">Lifting Capacity</h3>
              ...
              {/* Hydraulic Type */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-6">Hydraulic Type</h3>
                <Controller
                  name="hydraulicType"
                  control={control}
                  render={({ field }) => (
                ...
                {errors?.hydraulicType && (
                  <p className="mt-2 text-sm text-red-600">{errors.hydraulicType.message}</p>
                )}
              </div>

              {/* Control Type */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-6">Control Type</h3>
                <Controller
                  name="controlType"
                  control={control}
                  render={({ field }) => (
                ...
                />
                {errors?.controlType && (
                  <p className="mt-2 text-sm text-red-600">{errors.controlType.message}</p>
                )}
              </div>

              {/* Remote Valve */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-6">Remote Valve (Spool)</h3>
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div className="rounded-xl border border-gray-200 p-5">
                    <h4 className="font-medium text-gray-900 mb-4">Valve Type</h4>
                    <div className="space-y-3">
                      {remoteValveOptions.map((option) => (
                        <label key={option.value} className="flex cursor-pointer items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                          <input 
                            type="radio" 
                            value={option.value} 
                    ...
                    {errors?.remoteValveType && (
                      <p className="mt-2 text-sm text-red-600">{errors.remoteValveType.message}</p>
                    )}
                  </div>

                  <div className="rounded-xl border border-gray-200 p-5">
                    <h4 className="font-medium text-gray-900 mb-1">Number of Remote Valves</h4>
                    <p className="text-xs text-gray-500 mb-4">How many remote valves are available?</p>
                    <Controller
                      ...
                      render={({ field }) => (
                        <CustomListbox
                          ...
                          value={remoteValveCountOptions.find((o) => o.value === field.value) || null}
              ...
              {/* Linkage Details */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-6">Linkage Details</h3>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <CustomListbox
                        ...
                        data={linkageOptions}
                        value={linkageOptions.find((o) => o.value === field.value) || null}
                        onChange={(o) => field.onChange(o?.value)}
                        displayField="label"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <CustomListbox
                        ...
                        value={linkageCategoryOptions.find((o) => o.value === field.value) || null}
                        onChange={(o) => field.onChange(o?.value)}
                        displayField="label"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <CustomListbox
                        label="Top Link"
                        data={topLinkOptions}
                        value={topLinkOptions.find((o) => o.value === field.value) || null}
                        onChange={(o) => field.onChange(o?.value)}
                        displayField="label"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <CustomListbox
                        ...
                        value={draftSensitivityOptions.find((o) => o.value === field.value) || null}
                        onChange={(o) => field.onChange(o?.value)}
                        displayField="label"
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-6">Additional Hydraulic Features</h3>
