...
// backend order -> same shape jo component pehle mock data se expect kar raha tha
const buildProductData = (order, item) => {
  const currentIdx = STATUS_ORDER.indexOf(order.orderStatus);
  const placedDateTime = new Date(order.createdAt).toLocaleString("en-IN", {
    day: "2-digit",
    month: "short",
    hour: "2-digit",
    minute: "2-digit",
  ...
  const steps = STATUS_ORDER.map((step, idx) => ({
    status:
      step === "PLACED"
        ? "Order Placed"
        : step === "SHIPPED"
  ...
  return {
    ...
    product: {
      ...
      brand: "", // backend order-item pe brand save nahi hota abhi
      price: Number(item.price),
      quantity: item.quantity,
      image: apiHelper.getImageUrl(item.image), // order-item pe image save nahi hoti abhi -> placeholder
      sku: item.partNumber,
    },
    tracking: {
...
const ProductTracking = () => {
  ...
  useEffect(() => {
    const fetchTracking = async () => {
      try {
        setIsLoading(true);
        const data = await apiHelper.get(`/web/orders/${orderId}`);
        if (data.success) {
          const item = data.order.items.find(
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="text-center bg-white p-8 rounded-2xl shadow-sm max-w-md w-full">
          ...
          <h2 className="text-xl font-bold mb-2">Unable to load tracking</h2>
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                {productData.tracking.status === "Delivered"
                  ...
                  : `Expected delivery: ${productData.tracking.estimated}`}
              </p>
            </div>
